...
class _AvatarGlowState extends State<AvatarGlow> with SingleTickerProviderStateMixin {
  ...
  late final _curve = CurvedAnimation(parent: controller, curve: widget.curve);
  late final Animation<double> _smallDiscAnimation = Tween(begin: (widget.endRadius * 2) / 6, end: (widget.endRadius * 2) * (3 / 4)).animate(_curve);
  late final Animation<double> _bigDiscAnimation = Tween(begin: 0.0, end: (widget.endRadius * 2)).animate(_curve);
  late final Animation<double> _alphaAnimation = Tween(begin: 0.30, end: 0.0).animate(controller);
    ...
    return AnimatedBuilder(
      ...
      builder: (context, widgetChild) {
        final decoration = BoxDecoration(
          shape: widget.shape,
          color: widget.glowColor.withValues(alpha: _alphaAnimation.value.clamp(0.0, 1.0)),
        );
        return SizedBox(
            ...
            children: <Widget>[
              widget.animate
                  ? AnimatedBuilder(
                      ...
                      builder: (context, widget) {
                        final num size = _bigDiscAnimation.value.clamp(0.0, double.infinity);
                        return Container(height: size as double?, width: size as double?, decoration: decoration);
                      },
                    )
              ...
              widget.animate && widget.showTwoGlows
                  ? AnimatedBuilder(
                      ...
                      builder: (context, widget) {
                        final num size = _smallDiscAnimation.value.clamp(0.0, double.infinity);
                        return Container(height: size as double?, width: size as double?, decoration: decoration);
                      },
                    )
                  : const SizedBox(height: 0.0, width: 0.0),
